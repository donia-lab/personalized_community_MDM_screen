...
times = ["01","02","03","04","Feces"]
desired_otus = otutable[otutable.columns[otutable.columns.str.contains(pat = 'HD1')]].copy()
...
abundant_otus.rename(columns = {'GAM.03': 'mGAM.03'},inplace=True)
abundant_otus.rename(columns = {'GAM.04': 'mGAM.04'},inplace=True)
...
rect = patches.Rectangle((0.5,-0.02),4,1.04,linewidth=1.5,edgecolor='k',facecolor='none',clip_on=False,linestyle='--')
ax.add_patch(rect)

#Build JS inset axis
axins = axfig.add_axes([0.127, 0.93, 0.8, 0.1])
...
axfig.savefig('PD_family_level_all_days_figure.eps',dpi=600,bbox_inches = 'tight',format = 'eps')
